...
mps_organic_farm (int x, int y)
{
	...
	tmp=MP_INFO(x,y).int_1 * 100.0 / MAX_TECH_LEVEL;
	mps_store_sfp(i++,_("Tech"), tmp);

	tmp=MP_INFO(x,y).int_4 * 100.0 / 1200.0;
	mps_store_sfp(i++,_("Prod"), tmp);
}
